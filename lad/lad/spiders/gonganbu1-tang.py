...
class newsSpider(BaseTimeCheckSpider):
    ...
    start_urls = ['http://www.mps.gov.cn/n2253534/n2253535/index.html']

    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//*[@id="comp_5097045"]/dl//span/text()').extract()
        times = []
        #对时间进行特殊处理，去掉括号和空格
        for time_each in times_ori:
            times.append(time_each[2:-2])
        #是相对链接
        urls_ori = response.xpath('//*[@id="comp_5097045"]/dl//a/@href').extract()
        urls = []
        #变为绝对链接，下面的变化代码被注释
        ...
        next_requests = list()
        if should_deep:
            data_ori = response.xpath('//*[@id="pag_5097045"]').extract()[0]
            maxPageNum = re.findall('maxPageNum = (.*?);', data_ori)[0]
            next_url = "http://www.mps.gov.cn/n2253534/n2253535/index_5097045_" + str(int(maxPageNum)-1) + ".html"
