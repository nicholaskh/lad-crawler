...
class newsSpider(BaseTimeCheckSpider):
    name = 'haimenlaolingwang'
    start_urls = ['http://www.hmll.org/list/?1_1.html']

    def parse(self, response):
        should_deep = True
        times = response.xpath('//ul[@class="postlist"]//li/span/text()').extract()
        if len(times) == 0:
            should_deep =False

        #是相对链接
        urls_ori = response.xpath('//ul[@class="postlist"]//li/h3/a/@href').extract()
        urls = []
        for each in urls_ori:
            url = 'http://www.hmll.org' + each
            urls.append(url)
    ...
    def parse_info(self, response):
        ...
        #     title = title + each
        item["title"] = response.xpath('//h1[@class="midtitle"]/text()').extract()[0]

        item["sourceUrl"] = response.url
        # 修改了text_list
        #text_list = response.xpath('//*[@id="ivs_content"]/p/text() | //*[@id="ivs_content"]/p//font/text()')
        text_list = response.xpath('//div[@class="article"]/*')
