...
class newsSpider(BaseTimeCheckSpider):
    name = "huaianqulaolingwang2"
    start_urls = ['http://admin.zghaq.gov.cn/haqllw/zcfg/list.html']

    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//div[@class="s-lb-zw"]//div[@class="lb"]/text()').extract()
        if len(times_ori) == 0:
            should_deep = False
        times = []
        ...
        # 是相对链接
        urls_ori = response.xpath('//div[@class="s-lb-zw"]//div[@class="lb"]/a/@href').extract()
        urls = []
        for each in urls_ori:
            url = 'http://admin.zghaq.gov.cn/haqllw/' + each
            urls.append(url)
    ...
    def parse_info(self, response):
        ...
        item["source"] = "淮安区老龄网"
        item["title"] =  response.xpath('//td[@align="center"]/h3/text()').extract()[0].strip()
        item["sourceUrl"] = response.url
        # 修改了text_list
        # text_list = response.xpath('//*[@id="ivs_content"]/p/text() | //*[@id="ivs_content"]/p//font/text()')
        text_list = response.xpath('//td[@valign="top"]//p[position()>1]')
        ...
        for img in img_list:
            if 'http' not in img:
                img = 'http://admin.zghaq.gov.cn/haqllw/' + img
