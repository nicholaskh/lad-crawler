...
class newsSpider(BaseTimeCheckSpider):
    ...
    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//*[@id="left"]/div/ul//span/text()').extract()
        times = []
        ...
        #是相对链接
        urls_ori = response.xpath('//*[@id="left"]/div/ul//a/@href').extract()
        urls = []
        for each in urls_ori:
            url = 'http://www.jnmsjw.gov.cn' + each
            urls.append(url)
        ...
        if should_deep:
            ...
            next_url = 'http://www.jnmsjw.gov.cn/channels/209_' + str(nextPageNum) + '.html'
    ...
    def parse_info(self, response):
        ...
        item["city"] = "济南公安网"
        item["title"] = response.xpath('//*[@id="content-title"]/h3/text()').extract()[0]
        ...
        for img in img_list:
            if 'http' not in img:
                img = 'http://www.jnmsjw.gov.cn' + img
