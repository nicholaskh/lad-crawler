...
class newsSpider(BaseTimeCheckSpider):
    name = "ningxialaolingban2"
    start_urls = ['http://nxlaoling.com/index.php?f=cate&catid=10&page=1']

    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//div[@class="active_md"]//li/span/text()').extract()
        if len(times_ori) == 0:
            should_deep = False
        times = []
        ...
        # 是相对链接
        urls_ori = response.xpath('//div[@class="active_md"]//li/h3/a/@href').extract()
        urls = []
        for each in urls_ori:
            url = 'http://nxlaoling.com' + each
            urls.append(url)
    ...
    def parse_info(self, response):
        ...
        item["source"] = "宁夏回族自治区老龄工作委员会办公室网站"
        item["title"] = response.xpath('//h1[@class="big_title"]/text()').extract()[0].strip()
        item["sourceUrl"] = response.url
        # 修改了text_list
        # text_list = response.xpath('//*[@id="ivs_content"]/p/text() | //*[@id="ivs_content"]/p//font/text()')
        text_list = response.xpath('//div[@id="font_area"]/*')
