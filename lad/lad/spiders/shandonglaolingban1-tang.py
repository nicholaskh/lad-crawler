...
class newsSpider(BaseTimeCheckSpider):
    name = 'shandonglaolingban1'
    start_urls = ['http://www.sdpca.org.cn/zcfg.php?page=1']

    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//div[@class="cTmain"]/ul[2]//li/span/text()').extract()
        if len(times_ori) == 0:
            should_deep =False
        times = []
        for each in times_ori:
            times.append('20' + each)

        #是相对链接
        urls_ori = response.xpath('//div[@class="cTmain"]/ul[2]//li/a/@href').extract()
        urls = []
        for each in urls_ori:
            url = 'http://www.sdpca.org.cn/' + each
            urls.append(url)
        ...
        if should_deep:
            ...
            next_url = 'http://www.sdpca.org.cn/zcfg.php?page=' + str(nextPageNum)
