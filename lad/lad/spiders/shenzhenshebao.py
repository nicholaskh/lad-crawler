...
class newsSpider(BaseTimeCheckSpider):
    name = 'shenzhenshebao'
    start_urls = ['http://www.szsi.gov.cn/sbjxxgk/zcfggfxwj/ylbx/']

    def parse(self, response):
        should_deep = True
        times = response.xpath('//ul[@class="ftdt-list"]//li/a/em/text()').extract()
        if len(times) == 0:
            should_deep =False

        #是相对链接
        urls_ori = response.xpath('//ul[@class="ftdt-list"]//li/a/@href').extract()
        urls = []
