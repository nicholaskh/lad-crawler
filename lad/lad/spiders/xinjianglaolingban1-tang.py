...
class newsSpider(BaseTimeCheckSpider):
    name = "xinjianglaolingban1"
    start_urls = ['http://xj.cncaprc.gov.cn/channels/109.html']

    def parse(self, response):
        should_deep = True
        times_ori = response.xpath('//div[@class="box3"]//ul/li/span/text()').extract()
        if len(times_ori) == 0:
            should_deep = False
        times = []
        ...
        # 是相对链接
        urls = response.xpath('//div[@class="box3"]//ul/li/a/@href').extract()
        # urls = []
    ...
    def parse_info(self, response):
        ...
        item["source"] = "新疆维吾尔自治区老龄工作委员会办公室网站"
        item["title"] = response.xpath('//p[@class="box2title"]/text()').extract()[0]
        item["sourceUrl"] = response.url
        # 修改了text_list
        # text_list = response.xpath('//*[@id="ivs_content"]/p/text() | //*[@id="ivs_content"]/p//font/text()')
        text_list = response.xpath('//div[@class="box3"]//p')
